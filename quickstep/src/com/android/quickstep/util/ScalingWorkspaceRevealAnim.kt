...
class ScalingWorkspaceRevealAnim(
    ...
    companion object {
        ...
        val SCALE_INTERPOLATOR_DEFAULT = 
            PathInterpolator(
                Path().apply {
                    moveTo(0f, 0f)
        ...
        @JvmStatic
        fun getScaleInterpolator(context: Context): Interpolator {
            val isOemStyle = LauncherPrefs.get(context).get(LauncherPrefs.OEM_STYLE_LAUNCH_ANIM)
            return if (isOemStyle) SCALE_INTERPOLATOR_OEM else SCALE_INTERPOLATOR_DEFAULT
        }

        @JvmStatic
        fun getMinSize(context: Context): Float {
            val isOemStyle = LauncherPrefs.get(context).get(LauncherPrefs.OEM_STYLE_LAUNCH_ANIM)
            return if (isOemStyle) 0.92f else 0.85f
        }
    }

    private val isOemStyle = LauncherPrefs.get(launcher).get(LauncherPrefs.OEM_STYLE_LAUNCH_ANIM)
    private val scaleDurationMs = if (isOemStyle) 800L else 1000L
    private val fadeDurationMs = if (isOemStyle) 150L else 200L
    private val minSize = if (isOemStyle) 0.92f else 0.85f
    private val scaleInterpolator = if (isOemStyle) SCALE_INTERPOLATOR_OEM else SCALE_INTERPOLATOR_DEFAULT

    private val animation = PendingAnimation(scaleDurationMs)
    ...
    init {
        ...
        if (playAlphaReveal) {
            ...
            val fadeClamp = fadeDurationMs.toFloat() / scaleDurationMs
        ...
        transitionConfig.duration = scaleDurationMs
        ...
        if (playBlur) {
            ...
            transitionConfig.setInterpolator(StateAnimationConfig.ANIM_DEPTH, scaleInterpolator)
